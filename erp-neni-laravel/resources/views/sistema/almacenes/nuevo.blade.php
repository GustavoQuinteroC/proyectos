...
@section('content_header')
    <h1 class="m-0 text-dark">Registro de almacén</h1>
@stop

@section('content')
    <section class="content">
        <p>Ingrese la información del almacén</p>
        <div class="card">
            @if (session()->has('message') && session('message') == 'ok')
                <x-adminlte-alert theme="success" title="GUARDADO!">
                    El almacen se ha guardado de manera exitosa!
                </x-adminlte-alert>
            @endif
            <div class="card-body">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function actualizarTabla(productos) {
                ...
                tablaProductos.empty(); // Limpiar la tabla antes de agregar los nuevos productos
                productos.forEach(function(producto) {
                    var fila = $('<tr>');
                    fila.append('<td>' + producto.id + '</td>');
                    fila.append('<td>' + producto.catalogo + '</td>');
                    fila.append('<td>' + producto.descripcion + '</td>');
                    // Agregar icono de eliminación centrado en la última columna
